...
public class X2GameMode implements Screen, InputProcessor {
    Application game;
    ...
    private float GAME_SPEED = 1.75f;

    private Sprite background;

    public X2GameMode(Application game) {
        this.game = game;
        ...
        background = new Sprite(game.hardModeBackground);
        background.setPosition(0,-10);
        background.setSize(512,310);
    }
    ...
    public void show() {
        ...
        wall = new Wall(game.SPEED *GAME_SPEED,game); //START BLOCK
        start_corridor = SPAWN + wall.getBLOCK_SIZE() + 256;
        corridor = new Corridor(game,game.SPEED * GAME_SPEED);
        ...
        laser = new Laser(game, game.SPEED * GAME_SPEED); //Lasers
        bigArrow = new BigArrow(game, game.SPEED  *(-6 *GAME_SPEED));
        circle = new Circle(game, game.SPEED *GAME_SPEED);
        smallArrowZone = new SmallArrowZone(game,game.SPEED *(2 *GAME_SPEED));
        sniperZone = new SniperZone(game.SPEED *GAME_SPEED);
    ...
    public void render(float delta) {
        ...
        glyphLayout.setText(game.borderFont, ""+ HardScore.getGameScore() , Color.WHITE,512*2, Align.center, true);
    ...
    public void checkScoreAchievements(){
        if(game.android) {
            if (HardScore.hardScore == 14 && HardScore.bestHardScore < 14)
                game.gpgsController.unlockAchievement(game.achievement_not_bad_h1);

            if (HardScore.hardScore == 25 && HardScore.bestHardScore < 25)
                game.gpgsController.unlockAchievement(game.achievement_keep_going_h1);

            if (HardScore.hardScore == 50 && HardScore.bestHardScore < 50)
                game.gpgsController.unlockAchievement(game.achievement_carry_on_h1);

            if (HardScore.hardScore == 120 && HardScore.bestHardScore < 120)
                game.gpgsController.unlockAchievement(game.achievement_face_the_wind_h1);

            if (HardScore.hardScore == 200 && HardScore.bestHardScore < 200)
                game.gpgsController.unlockAchievement(game.achievement_who_are_you_h1);
        }
    }
    public void submitScore(){
        game.gpgsController.submitHardScore(HardScore.hardScore);
        game.gpgsController.submitMoney(Currency.maxMoney);
    }
    ...
    private void zoneCreator(){
        if(player.alive) {
            if (player.getPlayerRectangle().overlaps(corridor.getEndZone()) && !startEndCheck) {
                ...
                if(HardScore.hardScore > 14 && !game.openMoon) {
                    Application.openMoon = true;

                    game.preferences.putBoolean("openMoon", Application.openMoon);
                    game.preferences.flush();

                    game.gpgsController.unlockAchievement(game.achievement_here_we_go_again);
                }
            }
    ...
    private void collisionCheck(float delta){
        ...
        if(startZoneStart) {
            ...
            for (int i = 0; i < wall.getMassiveRect().length; i++) {
                if(Application.playerSkinHard == 0) { //others
                ...
                } else if(Application.playerSkinHard == 1) { //moon
        ...
        if(player.getPosition().y <= -64*2){
            // android
            if(!secondChance && Application.loadedReborn  && HardScore.hardScore >= 11) {
        ...
        if(player.alive && !startAfterDeath & !startPlay) {
            switch (keycode) {
                case Input.Keys.SHIFT_LEFT:
                    game.setScreen(new X2GameMode(game));
                    break;
                case Input.Keys.Z:
